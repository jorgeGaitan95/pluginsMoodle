 ...
 */
    ...
    if($quizid){
      if (! $quizrecord = $DB->get_record("quiz", array("id" => $quizid))) {
          print_error('invalidforumid', 'forum');
      }
      if (! $course = $DB->get_record("course", array("id" => $quizrecord->course))) {
          print_error('coursemisconf');
      }
      if (!$cm = get_coursemodule_from_instance("quiz", $quizrecord->id, $course->id)) {
      ...
      if (! $quizrecord = $DB->get_record("quiz", array("id" => $cm->instance))) {
          print_error('invalidassignid', 'assign');
      }
      //$activityid=get_activityid($quizrecord->id);
      $coursecontext = context_course::instance($course->id);
      require_login($course);
    }else {
    ...
    $timenow = time();
    $quizobj = quiz::create($quizrecord->id, $USER->id);
    ...
    $viewobj->startattempturl = quiz_start_attempt_url($cm,0,$activityid);
