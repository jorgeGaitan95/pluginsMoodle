 ...
 */
    ...
    $PAGE->set_url('/local/estrategia_didactica/post.php', array(
            'reply' => $reply,
            'forum' => $forum,
            'edit'  => $edit,
            'delete'=> $delete,
    ...
    /*if (!empty($forum)) {      // User is starting a new discussion in a forum
        ...
        if (! $parent = get_post_full_forum($reply)) {
            print_error('invalidparentpostid', 'forum');
        }
        if (! $discussion = $DB->get_record("forum_discussions", array("id" => $parent->discussion))) {
            print_error('notpartofdiscussion', 'forum');
        ...
        /*if (!user_can_post($forum, $discussion, $USER, $course, $modcontext)) {
            if (!isguestuser()) {
    ...
    if ($mform_post->is_cancelled()) {
        ...
        if (empty($SESSION->fromurl)) {
            $errordestination = "$CFG->wwwroot/local/estrategia_didactica/forumview.php?id=$forum->id";
        } else {
