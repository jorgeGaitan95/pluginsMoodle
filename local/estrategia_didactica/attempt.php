 ...
 */
 ...
 if ($id = optional_param('id', 0, PARAM_INT)) {
     redirect($CFG->wwwroot . '/local/estrategia_didactica/startattempt.php?cmid=' . $id . '&sesskey=' . sesskey());
 } else if ($qid = optional_param('q', 0, PARAM_INT)) {
     if (!$cm = get_coursemodule_from_instance('quiz', $qid)) {
         print_error('invalidquizid', 'quiz');
     }
     redirect(new moodle_url('/local/estrategia_didactica/startattempt.php',
 ...
 echo $output->quiz_notices($messages);
